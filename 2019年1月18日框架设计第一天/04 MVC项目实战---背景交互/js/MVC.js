...
var MVC = (function() {
	...
	return {
		addModel: function(path, value) {
			console.log("调用addModel", "A");
			M.add(path, value);
		},
		addView: function(name, handler) {
			console.log("调用addView", "C");
			V.add(name, handler);
		},
		addCtrl: function(name, handler) {
			
			console.log("调用addCtrl", "E");
			C.add(name, handler);
		},
		install: function() {
			console.log("启动所有控制器");
